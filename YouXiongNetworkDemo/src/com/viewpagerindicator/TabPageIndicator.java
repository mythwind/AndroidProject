...
public class TabPageIndicator extends HorizontalScrollView implements PageIndicator {
    ...
    private final OnFocusChangeListener mFocusChangeListener = new OnFocusChangeListener() {
		...
		public void onFocusChange(View v, boolean hasFocus) {
			if(hasFocus && v instanceof TabItem)
			{
				TabItem tab = (TabItem) v;
				setCurrentItem(tab.getIndex());
				if(mTabFocusChangeListener!=null)
				{
					mTabFocusChangeListener.onFocusChangeListener(tab.getIndex());
				}
			}
		}
    };
    ...
    private TabFocusChangeListener mTabFocusChangeListener;
    ...
    public void setOnTabFocusChangeListener(TabFocusChangeListener listener)
    {
    ...
    private void addTab(int index, CharSequence text, int iconResId,int tabId) {
        ...
        if(TextUtils.isEmpty(text))
        {
        	if(iconResId != 0)
        	{
        		final TabImageView tabView = new TabImageView(getContext());
        		tabView.setImageResource(iconResId);
        		tabView.setPadding(3, 3, 3, 3);
        ...
        {
        	final TabTextView tabView = new TabTextView(getContext());
            tabView.mIndex = index;
            tabView.setFocusable(true);
            tabView.setOnClickListener(mTabClickListener);
            tabView.setOnFocusChangeListener(mFocusChangeListener);
    ...
    private interface TabItem{
    	int getIndex();
    };
    
    private class TabImageView extends ImageView implements TabItem{
    	private int mIndex;
    ...
    private class TabTextView extends TextView implements TabItem{
        private int mIndex;
    ...
    /**
     * the viewpager's adapter implements this interface so that each tab view has a view id 
     */

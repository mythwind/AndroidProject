...
public class DirectionalViewPager extends ViewPager {
    ...
    public void setAdapter(PagerAdapter adapter) {
        ...
        if (mAdapter != null) {
            ...
            if (mRestoredCurItem >= 0) {
                ...
                setCurrentItemInternal2(mRestoredCurItem, false, true);
                mRestoredCurItem = -1;
    ...
    public void setCurrentItem(int item) {
        mPopulatePending = false;
        setCurrentItemInternal2(item, true, false);
    }

    void setCurrentItemInternal2(int item, boolean smoothScroll, boolean always) {
        ...
        if (smoothScroll) {
            if (mOrientation == HORIZONTAL) {
            	smoothScrollTo2(getWidth() * item, 0);
            } else {
            	smoothScrollTo2(0, getHeight() * item);
            }
     ...
     */
    void smoothScrollTo2(int x, int y) {
        if (getChildCount() == 0) {
            // Nothing to do.
            setScrollingCacheEnabled(false);
            return;
    ...
    void addNewItem2(int position, int index) {
    ...
    void dataSetChanged2() {
        ...
        if (newCurrItem >= 0) {
        	...
        	setCurrentItemInternal2(newCurrItem, false, true);
            needPopulate = true;
        }
        if (needPopulate) {
        	populate2();
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        if (mAdapter != null) {
            ...
            setCurrentItemInternal2(ss.position, false, true);
        } else {
    ...
    ItemInfo infoForChild2(View child) {
        for (int i = 0; i < mItems.size(); i++) {
            ItemInfo ii = mItems.get(i);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            ItemInfo ii;
            if (child.getVisibility() != GONE && (ii = infoForChild2(child)) != null) {
                ...
                if (mIsBeingDragged) {
                    ...
                    if ((Math.abs(initialVelocity) > mMinimumVelocity)
                        ...
                        if (lastMotion > mInitialMotion) {
                        	setCurrentItemInternal2(mCurItem - 1, true, true);
                        } else {
                        	setCurrentItemInternal2(mCurItem + 1, true, true);
                        }
                    } else {
                    	setCurrentItemInternal2(mCurItem, true, true);
                    }

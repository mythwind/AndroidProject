	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (info != null) {
			...
			if (!info.isAddButton()) {
				...
				Picasso.with(mContext).load(imageFile).placeholder(R.drawable.camerasdk_pic_loading)
						.error(R.drawable.camerasdk_pic_loading).resize(90, 90).centerCrop().into(holder.image);
			} else {
				// holder.image.setImageResource(R.drawable.ic_add_image_button);

				Picasso.with(mContext).load(R.drawable.ic_add_image_button)
						.placeholder(R.drawable.camerasdk_pic_loading).error(R.drawable.camerasdk_pic_loading)
						.resize(90, 90).centerCrop().into(holder.image);
			}
		}

...
public class StickerAdapter extends CommonListAdapter<Filter_Sticker_Info> {
	...
	public void getCommonView(ViewHolder helper, Filter_Sticker_Info item) {
		...
		Picasso.with(mContext).load(item.getImage()).placeholder(R.drawable.camerasdk_pic_loading)
				.error(R.drawable.camerasdk_pic_loading).resize(120, 120).centerCrop().into(image);
